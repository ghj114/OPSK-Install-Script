...
do
	case $1 in
        -a | --all) all=true; ;; -r | --rabbit) rabbitmq=true; ;;
        -m | mysql) mysql=true; ;; -k | keystone) keystone=true; ;; -g | glance) glance=true; ;;
        -c | controller) controller=true; ;; -p | compute) compute=true; ;; -d | dashboard) dashboard=true; ;;
	    *) echo "unrecognized:$1"; echo "Usage: openSt_instl.sh mysql keystone ..."; exit -1; ;;
	esac
	shift
done

#echo $all; echo $mysql; echo $keystone; echo $glance; echo $controller; echo $compute; echo $dashboard; exit 0
apt-get update
apt-get -y upgrade 

# mysql
if [ "$all" = "true" ] || [ "$mysql" = "true" ]; then ./mysql.sh;fi

# keystone
if [ "$all" = "true" ] || [ "$keystone" = "true" ]; then ./keystone.sh;fi

# glance
if [ "$all" = "true" ] || [ "$glance" = "true" ]; then ./glance.sh;fi;

# rabbitmq
if [ "$all" = "true" ] || [ "$rabbitmq" = "true" ]; then apt-get install -y rabbitmq-server; echo "rabbitmq install over!";sleep 1;fi

# controller
if [ "$all" = "true" ] || [ "$controller" = "true" ]; then ./nova-controller.sh;fi

# compute
#if [ "$all" = "true" ] || [ "$compute" = "true" ]; then ./nova-compute.sh;fi

# dashboard
if [ "$all" = "true" ] || [ "$dashboard" = "true" ]; then ./dashboard.sh;fi
